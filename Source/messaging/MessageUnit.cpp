    ...
    namespace Messaging {
        ...
        */
        uint32_t MessageUnit::Open(const string& pathName, const Settings::Config& configuration, const bool background, const flush flushMode)
        {
            ...
            _dispatcher.reset(new MessageDispatcher(*this, identifier, 0, _settings.BasePath().c_str(), _settings.MetadataBufferSize(), _settings.SocketPort()));
            ASSERT(_dispatcher != nullptr);
        ...
        */
        uint32_t MessageUnit::Open(const uint32_t instanceId)
        {
            ...
            if (instanceId != static_cast<uint32_t>(~0)) {
                ...
                _dispatcher.reset(new MessageDispatcher(*this, _settings.Identifier(), instanceId, _settings.BasePath(),  _settings.MetadataBufferSize(), _settings.SocketPort()));
                ASSERT(_dispatcher != nullptr);
